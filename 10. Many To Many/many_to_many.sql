# MANY TO MANY:  TV SHOW & REVIEWERS
...
SELECT title AS unreviewed_series FROM series LEFT JOIN reviews r ON series.id = r.series_id WHERE series_id IS NULL;
SELECT title AS unreviewed_series FROM reviews RIGHT JOIN series on reviews.series_id = series.id WHERE series_id IS NULL;
